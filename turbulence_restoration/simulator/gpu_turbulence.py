    ...
    def from_severity(severity: str) -> "TurbulenceConfig":
        presets = {
            "weak": TurbulenceConfig(
                ...
                blur_sigma=(0.0, 0.5),
                scintillation_strength=(0.0, 0.001),
                temporal_scale=0.08,
            ),
        ...
        if severity not in presets:
            raise ValueError(
                f"Unknown severity: {severity}. Available: {list(presets)}"
            )
...
def _rand_range(
    ...
    return torch.empty(tuple(shape), device=device, dtype=dtype).uniform_(
...
def _hash3(
    ix: torch.Tensor, iy: torch.Tensor, iz: torch.Tensor, seed: int
) -> torch.Tensor:
    """
...
def _grad_dot(
    hash_value: torch.Tensor, dx: torch.Tensor, dy: torch.Tensor, dz: torch.Tensor
) -> torch.Tensor:
    ...
    gx = torch.where(
        (h == 1) | (h == 3) | (h == 5) | (h == 7), -torch.ones_like(gx), gx
    )

    gy = torch.where(
        (h == 0) | (h == 1) | (h == 8) | (h == 10), torch.ones_like(gy), gy
    )
    gy = torch.where(
        (h == 2) | (h == 3) | (h == 9) | (h == 11), -torch.ones_like(gy), gy
    )

    gz = torch.where((h == 4) | (h == 5) | (h == 8) | (h == 9), torch.ones_like(gz), gz)
    gz = torch.where(
        (h == 6) | (h == 7) | (h == 10) | (h == 11), -torch.ones_like(gz), gz
    )
...
def perlin3d(
    x: torch.Tensor, y: torch.Tensor, z: torch.Tensor, seed: int = 0
) -> torch.Tensor:
    """
...
def value_noise3d(
    ...
    D = max(
        4, int(torch.ceil(t_span / float(temporal_scale)).detach().cpu().item()) + 4
    )
...
def warp_image(
    x: torch.Tensor, flow: torch.Tensor, padding_mode: str = "border"
) -> torch.Tensor:
    """
    ...
    warped = F.grid_sample(
        x_flat, grid, mode="bilinear", padding_mode=padding_mode, align_corners=True
    )
    if original_dim == 5:
...
def gaussian_kernel1d(
    sigma: float, device: torch.device, dtype: torch.dtype, truncate: float = 3.0
) -> torch.Tensor:
    if sigma <= 1e-6:
...
def gaussian_blur2d_per_frame(
    ...
    if original_dim == 5:
        ...
        x_flat = x.reshape(B * K, C, H, W)
        sigma_flat = (
            torch.full((B * K,), float(sigma), device=x.device, dtype=x.dtype)
            if not torch.is_tensor(sigma)
            else sigma.reshape(B * K).to(x)
        )
    elif original_dim == 4:
        N, C, H, W = x.shape
        x_flat = x
        sigma_flat = (
            torch.full((N,), float(sigma), device=x.device, dtype=x.dtype)
            if not torch.is_tensor(sigma)
            else sigma.reshape(N).to(x)
        )
        B = K = None
    else:
        raise ValueError(f"Expected x with 4 or 5 dims, got {x.shape}")
...
class GPUTurbulenceSimulator(nn.Module):
    ...
    def forward(
        ...
        if clean_frames.dim() != 5:
            raise ValueError(
                f"clean_frames must be [K,C,H,W] or [B,K,C,H,W], got {clean_frames.shape}"
            )
        ...
        blur_sigma = _rand_range(
            self.config.blur_sigma, (B, K), device, dtype, generator
        )
        ...
        if squeeze_batch:
            distorted_out, flow_out, alpha_out, blur_out = (
                distorted[0],
                flow[0],
                alpha[0],
                blur_sigma[0],
    ...
    def generate_flow(
        self, B, K, H, W, timestamps, device, dtype, generator=None
    ) -> torch.Tensor:
        cfg = self.config
        ...
        if len(weights) < cfg.octaves or len(scales) < cfg.octaves:
            raise ValueError(
                "octave_weights and spatial_scales must have at least `octaves` values"
            )
        ...
        for octave_idx, (scale, weight) in enumerate(zip(scales, weights)):
            ...
            u = u + float(weight) * value_noise3d(
                ...
                timestamps,
                float(scale),
                float(cfg.temporal_scale),
                seed=cfg.seed_u + 101 * octave_idx,
                device=device,
                dtype=dtype,
                generator=generator,
            )
            v = v + float(weight) * value_noise3d(
                ...
                float(scale),
                float(cfg.temporal_scale),
                seed=cfg.seed_v + 103 * octave_idx,
                device=device,
                dtype=dtype,
                generator=generator,
            )
    ...
    def generate_scintillation(
        self, B, K, H, W, device, dtype, generator=None
    ) -> torch.Tensor:
        cfg = self.config
        beta = _rand_range(
            cfg.scintillation_strength, (B, K, 1, 1, 1), device, dtype, generator
        )
        if torch.all(beta <= 1e-8):
            return torch.ones(B, K, 1, H, W, device=device, dtype=dtype)

        noise = torch.randn(
            B, K, 1, H, W, device=device, dtype=dtype, generator=generator
        )
        ...
        std = flat.std(dim=3, keepdim=True, unbiased=False).clamp_min(1e-6)[
            :, :, :, :, None
        ]
